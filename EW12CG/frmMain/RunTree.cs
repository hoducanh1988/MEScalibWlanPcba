...
namespace EW12CG {
    public partial class frmMain : Form {
        ...
        private bool calibWlan() {
            ...
            File.WriteAllText("ew12cg_remain.dll", lblProduct.Text);
            ...
            if (isCalib5G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode5G(lbl_SwitchMode5G, lbl_SwitchMode5GTime);
                st.Stop();
            ...
            if (isCalib2G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode2G(lbl_SwitchMode2G, lbl_SwitchMode2GTime);
                st.Stop();
        ...
        private bool measureAtt() {
            ...
            lbl_mac_att.Text = "";
            File.WriteAllText("ew12cg_golden.dll", "");
            File.WriteAllText("ew12cg_verify.dll", "");
            ...
            if (isCalib5G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode5G(lbl_aSwitchMode5G, lbl_aSwitchMode5GTime);
                st.Stop();
                UpdateRtbAll(string.Format("\r\nThời gian chuyển mode calib 5G: {0} sec\r\n", st.ElapsedMilliseconds / 1000));
            ...
            if (isCalib2G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode2G(lbl_aSwitchMode2G, lbl_aSwitchMode2GTime);
                st.Stop();
                UpdateRtbAll(string.Format("\r\nThời gian chuyển mode calib 2G: {0} sec\r\n", st.ElapsedMilliseconds / 1000));
            ...
            //11. measure and calculate attenuation 2G
            if (isCalib2G) {
                lbl_Att2G.Text = "Waiting...";
                var tt = new Stopwatch();
                tt.Start();
                ...
                if (!r) {
                    UpdateRtbAll("[FAIL] Mạch golden không đúng!\n\n");
                    VNPT_TextDisplay.SetText(lblaResult, "FAIL");
                    lbl_Att2GTime.Text = $"{tt.ElapsedMilliseconds} ms";
                    lbl_Att2G.Text = r ? "Passed" : "Failed";
                    goto END;
                }
                UpdateRtbAll(string.Format("{0}{1}{2}\n", "Frequency(MHz)".PadRight(20, ' '), "pw_Anten1(dbm)".PadRight(20, ' '), "pw_Anten2(dbm)".PadRight(20, ' ')));
                foreach (var item in goldenStandardValues) {
        ...
        END:
            ...
            if (r) {
                ...
                File.WriteAllText("ew12cg_golden.dll", g_macEth0);
        ...
        private bool verifyAtt() {
            ...
            if (isCalib5G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode5G(lbl_vSwitchMode5G, lbl_vSwitchMode5GTime);
                st.Stop();
                UpdateRtbAll(string.Format("\r\nThời gian chuyển mode calib 5G: {0} sec\r\n", st.ElapsedMilliseconds / 1000));
            ...
            if (isCalib2G) {
                st.Reset(); st.Restart();
                r = _switchCalibMode2G(lbl_vSwitchMode2G, lbl_vSwitchMode2GTime);
                st.Stop();
                UpdateRtbAll(string.Format("\r\nThời gian chuyển mode calib 2G: {0} sec\r\n", st.ElapsedMilliseconds / 1000));
            ...
            if (isCalib2G) {
                ...
                UpdateRtbAll("[PASS] Suy hao trạm đạt tiêu chuẩn!\n\n");
                tt.Stop();
                lbl_Verify2GTime.Text = $"{tt.ElapsedMilliseconds} ms";
                lbl_Verify2G.Text = r ? "Passed" : "Failed";
            ...
            if (r) {
                ...
                if (z == false) {
                    this.Invoke(new Action(() => {
                        ...
                        File.AppendAllLines(AppDomain.CurrentDomain.BaseDirectory + "ew12cg_verify.dll", new string[] { l.Text });
                    }));
                }
        ...
        //ok
        private bool _switchCalibMode5G(Label lb_switch_5g, Label lb_switch_5g_time) {
            Stopwatch st = new Stopwatch();
            st.Start();
        ...
        //ok
        private bool _switchCalibMode2G(Label lb_switch_2g, Label lb_switch_2g_time) {
            Stopwatch st = new Stopwatch();
            st.Start();
            lb_switch_2g.Text = "Waiting...";
        ...
        END:
            st.Stop();
            lb_switch_2g_time.Text = $"{st.ElapsedMilliseconds} ms";
            lb_switch_2g.Text = r ? "Passed" : "Failed";
            return r;
        }
        ...
        private bool _calibAndVerify(bool is2G) {
            try {
                TreeTestStatus = 0;
                string xttFileName = g_mode == 0 ? (is2G ? PathXTTFile2G : PathXTTFile5G) : (is2G ? PathATTFile2G : PathATTFile5G);
                ...
                if (_testTree != null) {
                    ...
                    string golden_file = string.Format("ew12cg_Golden\\GOLDEN_{0}.txt", g_macEth0.ToUpper());
        ...
        bool _getGoldenStandardValues(string mac_ethernet) {
            ...
            string golden_file = string.Format("ew12cg_Golden\\GOLDEN_{0}.txt", mac_ethernet.ToUpper());
            string golden_file_full_name = Path.Combine(dir, golden_file);
